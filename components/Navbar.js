...
const Navbar = () => {
  ...
  return (
    <nav className='bg-gray-900 text-white shadow-lg shadow-white flex justify-between items-center px-4 md:h-16 flex-col md:flex-row'>
      ...
      {/* <ul classNameNameName='flex justify-between gap-4'>
        <li>Home</li>
        <li>Abount</li>
        <li>Project</li>
        <li>Sign Up</li>
      ...
      <div className='relative flex flex-col md:block gap-4'>
        {session && <><button onClick={()=>setShowdropdown(!showdropdown)}  onBlur={()=> {setTimeout(() => {
          ...
          <div id="dropdown" className={`z-10 ${showdropdown?"":"hidden"} absolute left-31.25 bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700`}>
        ...
        {session && <button type="read" className="text-white bg-linear-to-br from-purple-600 to-blue-500 hover:bg-linear-to-bl focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-4 py-2.5 text-center leading-5" onClick={() => { signOut() }}>Logout</button>}
        {!session && <Link href={"/login"}>
          <button type="read" className="text-white bg-linear-to-br from-purple-600 to-blue-500 hover:bg-linear-to-bl focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-4 py-2.5 text-center leading-5">Login</button></Link>}
      </div>
    </nav>
  )
}
